...
def sayHello():
    ...
    sname01 = ['안심역', '각산역', '반야월역', '신기역', '율하역', '용계역', '방촌역', '해안역', '동촌역', '아양교역', '동구청역', '동대구역', '신천역', '칠성시장역',
               '대구역', '중앙로역', '반월당역', '명덕역', '교대역', '영대병원역',
               '현충로역', '안지랑역', '대명역', '서부정류장역', '송현역', '월촌역', '상인역', '월배역', '진천역', '대곡역', '화원역', '설화명곡역']
    sname02 = ['영남대역', '임당역', '정평역', '사월역', '신매역', '고산역', '대공원역', '연호역', '담티역', '만촌역', '수성구청역', '범어역', '대구은행역', '경대병원역',
               '반월당역', '청라언덕역', '반고개역', '내당역', '두류역',
               '감삼역', '죽전역', '용산역', '이곡역', '성서산업단지역', '계명대역', '강창역', '대실역', '다사역', '문양역']
    sname03 = ['용지역', '범물역', '지산역', '수성못역', '황금역', '어린이회관역', '수성구민운동장역', '수성시장역', '대봉교역', '건들바위역', '명덕역', '남산역', '신남역',
               '서문시장역', '달성공원역', '북구청역', '원대역',
    ...
    holiday = [pytimekr.newyear(), pytimekr.lunar_newyear(), pytimekr.samiljeol(), pytimekr.buddha(),
               pytimekr.children(), pytimekr.memorial(), pytimekr.independence(),
    ...
    if not timestamp1 and timestamp2:
        ...
        print(desti1 + "방면 도착 예정 시간은 " + timestamp1[0][0] + " 시 " + timestamp1[0][1] + " 분 입니다.")
        print(desti2 + "방면 도착 예정 시간은 " + timestamp2[0][0] + " 시 " + timestamp2[0][1] + " 분 입니다.")
        output = desti1 + "방면 도착 예정 시간은 " + timestamp1[0][0] + " 시 " + timestamp1[0][
...
def line3_arrtm():
    ...
    url = "http://openapi.kric.go.kr/openapi/trainUseInfo/subwayTimetable?serviceKey=$2a$10$jdJGVD.bxBIfm1cbCh.UVOd6Bg.LAks9WIpYQjrBoGiMzqQNmZeXu&format=xml&railOprIsttCd=DG&dayCd=%s&lnCd=%s&stinCd=%s" % (
    weekday, linecode, findname)
    print(url)
